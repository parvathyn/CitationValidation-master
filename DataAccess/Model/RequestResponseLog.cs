...
namespace DataAccess.Model
{
    public class RequestResponseLog
    {
                        ...
                        foreach (var item in data.VendorData)
                        {
                            switch (item.Key)
                            {
                                ...
                                case VendorNames.Pango:
                                    ...
                                    break;
                                case VendorNames.MiXchange:
                                    var miexresponse = ((VendorResponseEntity)item.Value);
                                    if (miexresponse != null)
                                    {
                                        sb.Append(Environment.NewLine);
                                        sb.AppendFormat("Vendor  : {0} ", item.Key.ToString()).Append(Environment.NewLine);
                                        sb.AppendFormat("Http Request : {0}", miexresponse.RequestURL).Append(Environment.NewLine);
                                        sb.AppendFormat("Http Request Body : {0}", miexresponse.MiExRequestData).Append(Environment.NewLine);
                                        sb.AppendFormat("Response Body: {0}", miexresponse.MiExResponse).Append(Environment.NewLine);
                                        sb.AppendFormat("Http Status : {0}", miexresponse.ResponseHttpStatusCode.ToString()).Append(Environment.NewLine);

                                        if (miexresponse.GeneralException != null)
                                        {
                                            sb.AppendFormat("Exception  : {0} ", miexresponse.GeneralException.Message).Append(Environment.NewLine);
                                            if (miexresponse.GeneralException.InnerException != null)
                                            {
                                                sb.AppendFormat("inner Exception  : {0} ", miexresponse.GeneralException.InnerException.Message).Append(Environment.NewLine);
                                            }
                                        }
                                    }
                                    break;
                                case VendorNames.ParkMobile:
                                case VendorNames.ParkMobile2:
                                ...
                                case VendorNames.Cale:
                                    ...
                                    break;
                                case VendorNames.IPC:
                                    var ipcvresponse = ((VendorResponseEntity)item.Value);
                                    if (ipcvresponse != null)
                                    {
                                        sb.Append(Environment.NewLine);
                                        sb.AppendFormat("Vendor  : {0} ", item.Key.ToString()).Append(Environment.NewLine);
                                        sb.AppendFormat("Http Request : {0}", ipcvresponse.RequestURL).Append(Environment.NewLine);
                                        sb.AppendFormat("Http Body : {0}", ipcvresponse.IPCRequestData).Append(Environment.NewLine);
                                        sb.AppendFormat("Http Status : {0}", ipcvresponse.ResponseHttpStatusCode.ToString()).Append(Environment.NewLine);
                                        sb.AppendFormat("Response  : {0} ", ipcvresponse.XmlandJsonData).Append(Environment.NewLine);
                                        if (ipcvresponse.GeneralException != null)
                                        {
                                            sb.AppendFormat("Exception  : {0} ", ipcvresponse.GeneralException.Message).Append(Environment.NewLine);
                                            if (ipcvresponse.GeneralException.InnerException != null)
                                            {
                                                sb.AppendFormat("inner Exception  : {0} ", ipcvresponse.GeneralException.InnerException.Message).Append(Environment.NewLine);
                                            }
                                        }
                                    }
                                    break;
        ...
        public static async void PlateReqResLog(PlateNoRequestResponse data, string filepath)
        {
            try
            {
                if (!Directory.Exists(filepath))
                ...
                filepath = Path.Combine(filepath, data.Response.CurrentUTCTime.ToString("MM-dd-yy") + ".txt");   //Date in Customer Time
                if (!File.Exists(filepath))
                {
                    File.Create(filepath).Dispose();
                }
                using (StreamWriter sw = File.AppendText(filepath))
                {
                    ...
                    sb.AppendFormat("UTC Time : {0} ", data.Request.CurrentUTCTime.ToString("O")).Append(Environment.NewLine);
                    sb.AppendFormat("IP Address :{0} ", data.Request.IPAddress).Append(Environment.NewLine);
                    sb.AppendFormat("Request Data : {0} ", data.Request.RequestURL).Append("\t");
            ...
            {
                var appPath = System.Web.Hosting.HostingEnvironment.ApplicationPhysicalPath;
                string errorPath = Path.Combine(appPath, "GetPlateNo");
                ExceptionLogging.SendErrorToText(ex, "GetPlateNo", errorPath);
            }
        }
