...
namespace DataAccess.Model.Cale
{
    public class CaleGenericFactory
    {
        ...
        public TransactionTransform GetRefreshPlateDataArticle(ref VendorResponseEntity venderResonse)
        {
            TransactionTransform refreshPlate = new TransactionTransform() { ReturnCode = ((int)ReturnCodeEnum.Error).ToString() };
            try
            {
                            ...
                            if (data != null)
                            {
                                if (data.ValidParkingData.Count > 0)
                                {
                                    ...
                                    if (validData != null)
                                    {
                                        ...
                                        refreshPlate.MeterExpiredMinutes = this._parameter.ExpiredMinutes(this._customer.GracePeriodInSecond.ToNullableInt(), validData.EndDateUtc).ToString();
                                        //////
                                        if (validData.Article != null && !string.IsNullOrWhiteSpace(validData.Article.Name))
        ...
        public TransactionTransform GetRefreshPlateDataPhiladelphia(ref VendorResponseEntity venderResonse, string zone)
        {
            TransactionTransform refreshPlate = new TransactionTransform() { ReturnCode = ((int)ReturnCodeEnum.Error).ToString() };
            try
            {
                    ...
                    if (response.IsSuccessStatusCode)
                    {
                        try
                        {
                            ...
                            if (data != null)
                            {
                                if (data.ValidParkingData.Count > 0)
                                {
                                    ArrayOfValidParkingDataValidParkingData validData = data.ValidParkingData.Where( t=> t.Article.Name.ToLower()== zone.ToLower()).OrderByDescending(t => t.EndDateUtc).FirstOrDefault();
                                    if (validData != null)
                                    {
                                        refreshPlate.LicensePlate = validData.Code;
                                        refreshPlate.MeterExpiredMinutes = this._parameter.ExpiredMinutes(this._customer.GracePeriodInSecond.ToNullableInt(), validData.EndDateUtc).ToString();
