...
namespace DataAccess.Model.Parkeon
{
    public class ParkeonServiceAccess
    {
        ...
        public void ProcessRequest(ref TransactionTransform transformData, TypeConditions typeCondition, TimeZoneInfo TZ, ref  DateTime? currentTime, ref VendorResponseEntity venderResonse)
        {
            List<ParkeonResponsePlates> platePayments = null;
            try
            {
                ...
                //Get Data
                platePayments = this.GetPlatePurchases(startTime, endTime, keyParameter.PlateNumber, typeCondition, ref venderResonse);
        ...
        private List<ParkeonResponsePlates> GetPlatePurchases(string startTime, string endTime, string plateNumber, TypeConditions typeCondition, ref VendorResponseEntity venderResonse)
        {
            List<ParkeonResponsePlates> zonelist = new List<ParkeonResponsePlates>();
            try
            {
                ...
                try
                {
                    ...
                    using (var client = new HttpClient())
                    {
                        ...
                        var byteArray = Encoding.ASCII.GetBytes(string.Format("{0}:{1}", this.serviceParameters.userName, this.serviceParameters.userPassword));
                        client.DefaultRequestHeaders.Authorization = new System.Net.Http.Headers.AuthenticationHeaderValue("Basic", Convert.ToBase64String(byteArray));
                        venderResonse.RequestURL = this.serviceParameters.URI;
                        venderResonse.ParkeonRequstBody = reqObj.GetXmlTextData();
                        HttpResponseMessage response = client.PostAsync(this.serviceParameters.URI, httpContent).Result;
                        if (response.IsSuccessStatusCode)
                        {
                            string xmlData = response.Content.ReadAsStringAsync().Result;
