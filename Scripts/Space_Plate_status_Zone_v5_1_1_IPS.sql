...
--       [Space_Plate_status_Zone_v5_1_1_IPS]  7010 , '68A1BR' 
--Space_Plate_status_Zone_v5_1_1_IPS
;                
...
left join EnfVendorStall as e with(nolock) on p.CustomerId=e.EnfCustomerId and str(p.MeterId) = e.MeterID 
--NEW LINE: 
Left join EnfZone as z with(nolock) on  p.CustomerId=z.EnfCustomerId and Z.[Name] = cast(P.ZoneID as varchar)  
...
left join EnfVendorStall as e with(nolock) on p.EnfCustomerId=e.EnfCustomerId and p.PayStationId=e.PayStationId  
Left join EnfZone as z with(nolock) on e.EnfCustomerId=z.EnfCustomerId and e.Zoneid=z.ID  
...
FROM cte 
order by cte.PurchasedDate desc, cte.ExpiryTime desc , ENFCreatedDateTime desc , 
